...
old_collision="$OUT/funnel-summary-20200101T000000Z-2.dump.gpg"
printf old-collision > "$old_collision"
printf old-collision-digest > "$old_collision.sha256"
manual="$OUT/funnel-summary-manual-archive.dump.gpg"
printf manual > "$manual"
touch -d '30 days ago' \
	"$old" "$old.sha256" "$old_collision" "$old_collision.sha256" "$manual"
...
assert_missing "$old_collision" "expired collision-suffixed summary"
assert_missing "$old_collision.sha256" "expired collision-suffixed digest"
