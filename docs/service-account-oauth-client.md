...
subject and supplies its kind; no container restart is needed. Empty or revoked
admission rejects the token. The signed `gty` path still requires admission. See
[OAuth subject admission](oauth-subjects.md) for provisioning, migration,
revocation, and the split-Qubes administrator path.
...
3. Check the API's access-token profile. The default Auth0 profile emits the
   ...
   verified M2M subject enrolled with `kind=service`.
...
5. **Enroll the subject before expecting a successful smoke** — admission is
   ...
   value even though it ends in 401. Run
   `subject-admin allow '<exact-subject>' service '<agent-label>'` through the
   deployment's tools profile; admission applies on the next request. (The
   refused attempt is normally also recorded server-side in `mcp_auth_events`
   with `reason='subject_not_allowed'` and the verified subject — audit writes
   are best-effort, so when the row is present it is the server-verified
   cross-check for the locally decoded print.)
6. Run the smoke again; it should now report success. The enrolled `service`
   kind supplies attribution even without `gty`; the signed Auth0 claim remains
   supported and never bypasses admission.
...
401 — then enroll the reported subject with
`subject-admin allow
'<exact-subject>' service` and run the smoke again. No MCP
restart is required. Never infer the subject from a dashboard label: use the
exact value from the token the client actually minted (the helper's locally
decoded print), and — when the best-effort audit recorded the bootstrap attempt
— cross-check it against the server-verified copy on that `subject_not_allowed`
row.
...
There is intentionally no per-tool successful-read audit in this release: a
read-only service account's application-level trace is the request-level
admission row (`outcome='allowed'` with its verified subject — best-effort
delivery, so gaps are possible and self-announcing), plus Caddy request
metadata; which tools or rows it read is not recorded. Failed Bearer tokens
retain the existing `token_validation_failed` reason rather than a machine/user
label: claims from an unverified token are attacker-controlled and must not
enter trusted audit data. Provider-side logs remain the source of truth for
failed token exchanges, which occur before Open Brain receives a Bearer token.
...
| MCP returns 401                                                | First suspect on a valid tenant token: the subject has no active row in `oauth_auth.allowed_subject` (fail-closed admission) — the audit row normally reads `subject_not_allowed` with the verified subject (best-effort delivery). Otherwise check `iss`, `aud`, `exp`, `sub`, RS256, the configured JWKS URI, and clock skew. Open Brain deliberately returns the same public message for every failure; use the reason-coded auth audit and provider logs. |
...
| Authentication works but writes show `door=funnel`             | The issuer supplied no signed `gty=client-credentials`; enroll/update the exact verified `sub` with `subject-admin allow` and kind `service`.                                                                                                                                                                                                                                                                                                                 |
...
Brain validates them locally. To deny all tokens for that subject immediately,
use `subject-admin revoke`; Open Brain does not introspect individual JWTs.
Choose an access-token lifetime that bounds that residual window.

Deleting and recreating an application may change its `sub`. Re-run the
bootstrap smoke, enroll the new subject with `subject-admin allow`, smoke again,
then revoke the retired subject with `subject-admin revoke`. These changes take
effect on the next request without a restart. A changed `sub` creates a new
personal-memory principal; changing the label or re-enrolling the same subject
does not.
