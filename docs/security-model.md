...
SELECT/DELETE on that table only and direct, non-delegable `public` schema
USAGE, so hardened deployments need not retain PostgreSQL's default PUBLIC
USAGE. It has no sequence, database/schema `CREATE`, other corpus relation, or
privileged-function access. The completed-catalog assertion pins the app,
readonly, and rollup sides of the split and scans for membership, grant-option,
persistent-object, and sideways-access drift; the server boot probe mirrors that
boundary against post-deployment drift.
