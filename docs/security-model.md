...
| `openbrain_auth_rollup` | SELECT/DELETE on `mcp_auth_events` only; no INSERT/UPDATE, sequence access, other corpus relation access, privileged-function execution, unsafe cluster attributes, or role memberships                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | target-pinned corpus auth-event report and retention job                                                                                                                                                                                                                                                                                                                                                                                                       |
...
`openbrain_readonly`, and `openbrain_token_admin`. `openbrain_ingester`, `openbrain_monitor`,
`openbrain_logs_rollup`, and optional `openbrain_logs_backup` are roles on a
**separate cluster**: a socket-only Funnel-log sink whose only application data
tables are two permanent request-metadata tables (supporting sequences/indexes
...
`db/12-auth-audit-grants.sql` separates auth-decision production from
retention. `openbrain_app` can SELECT/INSERT `mcp_auth_events` but cannot UPDATE
or DELETE landed evidence. The standalone `openbrain_auth_rollup` role gets
SELECT/DELETE on that table only, with no sequence, other corpus relation, or
privileged-function access. The completed-catalog assertion pins both sides of
the split and scans the rollup role for sideways access; the server boot probe
refuses the historical broad application grant.
...
- The request-path `openbrain_app` role can SELECT/INSERT those rows but cannot
  ...
  cannot insert, rewrite, or access the rest of the corpus. The completed-
  catalog assertion and server boot probe pin the split.
...
- **Revision attribution is application-trusted, not database-attested.**
  ...
  compromised app credential cannot rewrite or erase genuine history, but it
  can append fabricated rows or attribution. Deriving those fields from custom
  transaction-local GUCs would not change that boundary because the app role
  can set the same GUCs. A stronger design would revoke direct history INSERT
  and couple revision creation to owner-controlled mutation functions or
  triggers, while also solving how PostgreSQL receives non-forgeable request
  identity. That complexity is deliberately deferred unless multi-tenant,
  compliance, or incident-response requirements make database-enforced
