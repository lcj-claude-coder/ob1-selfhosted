...
| `openbrain_auth_rollup` | Non-delegable SELECT/DELETE on `mcp_auth_events` only; no INSERT/UPDATE, sequence access, database/schema `CREATE`, other corpus relation access, privileged-function execution, unsafe cluster attributes, or role memberships                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | target-pinned corpus auth-event report and retention job                                                                                                                                                                                                                                                                                                                                                                                                       |
...
SELECT/DELETE on that table only, with no sequence, database/schema `CREATE`,
other corpus relation, or privileged-function access. The completed-catalog
assertion pins both sides of the split and scans the rollup role for grant
options, persistent-object creation, and sideways access; the server boot probe
mirrors that boundary against post-deployment drift.
