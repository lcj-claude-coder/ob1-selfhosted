...
| Client inside the range replaying a **stolen, still-valid** Bearer token                                                                                     | A stolen token works until it expires or the operator runs `subject-admin revoke` for its subject; the next request then fails. Revocation is subject-wide and blocks every token for that identity. There is no per-token introspection or individual JWT revocation. Before revocation, the token reaches shared workspace/project audiences and the stolen subject's personal rows. Requests (reads included) enqueue `outcome=allowed` audit rows with the subject, door, path, and client IP through the best-effort shared queue (security model § audit contract), so the abuse window is reconstructable afterwards up to audit gaps — drops are counted and warned, but a dropped event's identity is not recoverable | [`security-model.md`](security-model.md#trust-boundaries)                                    |
| IdP-side misconfiguration mints valid tokens for accounts the operator never admitted — an accidentally-enabled social connection, an unintended signup flow | The database-backed `oauth_auth.allowed_subject` admission table: a new account is a new `sub`, which fails authorization after every cryptographic check passes. The attempt is recorded with the verified subject (`subject_not_allowed`, best-effort). Admission fails closed — empty/revoked rows or a DB error reject the request; each request re-reads revocation — so the tenant dashboard is no longer the only gate                                                                                                                                                                                                                                                                                                  | [`security-model.md`](security-model.md#application-layer)                                   |
| Compromised OAuth tenant — the attacker can mint valid tokens **for allowlisted subjects**                                                                   | Honestly: full read/write — a token minted for an admitted `sub` trips no in-path control, and a tenant that can mint arbitrary claims can mint admitted ones. The allowlist bounds this to impersonation of known subjects (novel accounts are refused, previous row). Visibility: requests enqueue `outcome=allowed` audit rows keyed to the impersonated subject (best-effort — gaps possible and self-announcing, identities in a gap unrecoverable), retained 365 days                                                                                                                                                                                                                                                    | [`security-model.md`](security-model.md#trust-boundaries)                                    |
...
| Leaked M2M client secret                                                                                                                                     | It can mint service tokens immediately; this is the intended capability of `client_credentials`. Use one client per agent, a short access-token lifetime, provider-side rotation/revocation, tailnet routing, and the verified `service`/`sub` write stamps to bound and investigate exposure. Revoke the admitted subject immediately with `subject-admin revoke` to reject already-issued and newly minted tokens on their next request; rotate/revoke the provider secret as well to stop minting. Individual JWTs have no per-token introspection/revocation                                                                                                                                                               | [`service-account-oauth-client.md`](service-account-oauth-client.md#rotation-and-revocation) |
