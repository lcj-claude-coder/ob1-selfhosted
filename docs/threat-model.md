...
| IdP-side misconfiguration mints valid tokens for accounts the operator never admitted — an accidentally-enabled social connection, an unintended signup flow | The in-app `OAUTH_ALLOWED_SUBJECTS` allowlist: a new account is a new `sub`, which fails authorization after every cryptographic check passes. The attempt is recorded with the verified subject (`subject_not_allowed`, best-effort). The list fails closed — unset means nobody is admitted — so the tenant dashboard is no longer the only gate                                                                                                                                                                                                                                                    | [`security-model.md`](security-model.md#application-layer)                                   |
